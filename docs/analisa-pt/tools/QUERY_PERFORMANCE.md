...
| 2026-06-12 | (this commit) | n/a | n/a | n/a | Committed `add_adjudicatario_nif.py` — Tier 1 migration (adjudicatario_nif column + 3 composite indexes) now reproducible from a fresh clone. Run `python add_adjudicatario_nif.py` to rerun the column backfill + 3 CREATE INDEX on an existing procurement.db. Idempotent. |
